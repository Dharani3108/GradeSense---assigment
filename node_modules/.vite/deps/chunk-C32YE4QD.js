...
function useResyncOnReconnect(getInitialValue) {
  const _connected = import_react.default.useRef(false);
  const _initialized = import_react.default.useRef(false);
  const _prevValue = import_react.default.useRef(void 0);
  const _renderCount = import_react.default.useRef(0);
  _renderCount.current++;
  if (!_initialized.current && getInitialValue) {
    _initialized.current = true;
    _prevValue.current = cloneObject(getInitialValue());
  }
  const resyncIfNeeded = import_react.default.useCallback((enabled, getCurrentValue, setValue) => {
  ...
  const snapshot = import_react.default.useCallback((enabled, getCurrentValue) => {
    if (enabled) {
...
function useFormState(props) {
  ...
  const [formState, updateFormState] = import_react.default.useState(getCurrentFormState);
...
var generateWatchOutput = (names, _names, formValues, isGlobal, defaultValue) => {
  ...
  if (Array.isArray(names)) {
    return names.map((fieldName) => (isGlobal && _names.watch.add(fieldName), get(formValues, fieldName, get(defaultValue, fieldName))));
  }
...
function useWatch(props) {
  ...
  const [value, updateValue] = import_react.default.useState(getInitialOutput);
  ...
  const _refreshValue = import_react.default.useRef(refreshValue);
  _refreshValue.current = refreshValue;
  const _getCurrentOutput = import_react.default.useRef(getCurrentOutput);
  ...
  const shouldReturnImmediate = import_react.default.useMemo(() => {
    if (disabled) {
      return false;
    }
...
function useController(props) {
  ...
  const defaultValueMemo = import_react.default.useMemo(() => {
...
var getCheckboxValue = (options) => {
  ...
  return isUndefined(option.value) || option.value === "" ? validResult : { value: option.value, isValid: true };
};
var defaultReturn = {
  isValid: false,
  value: null
...
var isFileLike = (value) => typeof Blob !== "undefined" && value instanceof Blob || typeof File !== "undefined" && value instanceof File;
var flatten = (obj) => {
  const output = {};
...
var FormProvider = ({ children, watch, getValues, getErrors, getFieldState, setError, clearErrors, setValue, setValues, trigger, formState, resetField, reset, resetDefaultValues, handleSubmit, unregister, control, register, setFocus, subscribe }) => {
  ...
  const trigger = async (name, options = {}) => {
    ...
    if (options.shouldTouch) {
      for (const fieldName of name ? fieldNames : _names.mount) {
        !_names.array.has(fieldName) && set(_formState.touchedFields, fieldName, true);
      }
    }
    _subjects.state.next({
...
//# sourceMappingURL=chunk-C32YE4QD.js.map
